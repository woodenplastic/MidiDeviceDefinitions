...
function generateDeviceIcon(manufacturer, deviceName) {
    // Generate manufacturer_device format icon name
...
function mergeDeviceData(existingDevice, csvData, manufacturer, deviceName) {
    // Generate automatic device icon if not provided
    const autoGeneratedIcon = generateDeviceIcon(manufacturer, deviceName);
    
    // Create new device structure if it doesn't exist
    if (!existingDevice) {
        return {
            device_icon: autoGeneratedIcon,
            midi_thru: "",
            midi_in: "",
            midi_clock: "",
            phantom_power: "",
    ...
    // Preserve existing manual data, only update if empty or "none"
    const mergedDevice = {
        device_icon: (existingDevice.device_icon && existingDevice.device_icon !== "none") 
                     ? existingDevice.device_icon 
                     : autoGeneratedIcon,
...
function mergeParameters(existingParams, newParams, paramType) {
    // First, ensure all existing parameters have icon_number field
    const normalizedExistingParams = existingParams.map(param => ({
        ...param,
        icon_number: param.icon_number !== undefined ? param.icon_number : null
    }));
    ...
    if (paramType === 'cc') {
        normalizedExistingParams.forEach((param, index) => {
    ...
    const updatedParams = [...normalizedExistingParams];
    ...
    newParams.forEach(newParam => {
        ...
        if (existing) {
            ...
            updatedParams[existing.index] = {
                ...
                // Preserve existing icon_number or set to null if missing
                icon_number: existingParam.icon_number !== undefined ? existingParam.icon_number : null
            };
...
function convertDatabase() {
        ...
        csvFiles.forEach(file => {
            try {
                ...
                if (deviceData.length > 0) {
                    ...
                    // Merge device data (preserves existing manual edits and auto-generates icon)
                    const mergedDevice = mergeDeviceData(
                        database[manufacturer][deviceName], 
                        deviceData,
                        deviceData[0].manufacturer, // Original manufacturer name
                        deviceData[0].device        // Original device name
                    );
                    ...
                    // Log device icon generation if it was auto-created
                    if (mergedDevice.device_icon && (!deviceExists || !database[manufacturer][deviceName] || database[manufacturer][deviceName].device_icon === "none")) {
                        console.log(`    🎨 Auto-generated icon: "${mergedDevice.device_icon}"`);
                    }
    ...
    console.log('🎨 Device icons auto-generated in manufacturer_device format!');
}
